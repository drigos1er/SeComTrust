    ...
    for item in l_rpsdesc:
        ...
        if n==1: # ona un seul ami donc on est dans un contexte de FOF
            for rec in l_recom:
                ...
                if trust_up >= thresh[1] and trust_pu >= thresh[0] and l_slainf['qtyu'] == l_slainf['qtyp'] and \
                        l_slainf[
                    ...
                    trans_init = valini_tram.init_tram(trans_inits['idu'], trans_inits['idp'], tts, ttsp,
                                                       trans_inits['resource'], trans_inits['qtyu'],
                                                       trans_inits['qtyp'],
                                                       trans_inits['bmodeu'], trans_inits['bmodep'],
                                                       trans_inits['tp_u'],
                                                       trans_inits['tp_p'],
            ...
            tt = 0.7 * drt + 0.3 * rep_value.specrep_current(item[0])
            trust_up = tt
            trust_pu = rep_value.global_rep(o_u)
            l_slainf = rep_value.sla_info(o_u, item[0], des_res, sens_res, qty_res, date_res)
            al_u = rep_value.assurlev_current(o_u)
            ...
            sd_p = int_assulev.security_domain(al_p)
            thresh = gov_matrix.der_thresh(sd_p, sd_u)
            if trust_up >= thresh[1] and trust_pu >= thresh[0] and l_slainf['qtyu'] == l_slainf['qtyp'] and l_slainf[
                'bmodeu'] == l_slainf['bmodep']:
                trans_inits = l_slainf
                tts = trust_up
                ttsp = trust_pu
